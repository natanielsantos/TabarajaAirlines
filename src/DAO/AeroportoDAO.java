...
public class AeroportoDAO {
	...
	public void inserirNoBanco(Aeroporto aer) {
		try {
			PreparedStatement pst = con.prepareStatement("INSERT INTO aeroporto " + "VALUES (?,?,?)");
	...
	public void alterarNoBanco(Aeroporto aer) {
		try {
			PreparedStatement pst = con
					.prepareStatement("UPDATE aeroporto SET  nome = ?, " + "cidade = ? WHERE idaeroporto = ?");
	...
	public Aeroporto consultar(String cod) {
		...
		try {
			...
			if (rs.next()) {
				aer = new Aeroporto(rs.getString("idaeroporto"), rs.getString("nome"), new Cidade(rs.getInt("idcidade"),
						rs.getString("municipio"), rs.getString("pais"), rs.getString("estado")));
	...
	public ArrayList<Aeroporto> relatorio() {
		...
		try {
			...
			PreparedStatement pst = con.prepareStatement(
					"SELECT * FROM aeroporto INNER JOIN " + "cidade ON aeroporto.cidade = cidade.idcidade");

			rs = pst.executeQuery();

			while (rs.next()) {

				aeroportos.add(new Aeroporto(rs.getString("idaeroporto"), rs.getString("nome"),
