...
public class AviaoDAO {
	...
	public void inserirNoBanco(Aviao av) {
		try {
			PreparedStatement pst = con.prepareStatement("INSERT INTO aviao " + "VALUES (?,?,?,?,?,?)");
	...
	public void alterarNoBanco(Aviao av) {
		try {
			PreparedStatement pst = con.prepareStatement(
					"UPDATE aviao SET modelo = ?, " + "capac_passageiros = ?, capac_carga = ?, qtd_turbinas = ?, "
							+ "capac_combust_por_turbina = ? " + "WHERE identificacao = ?");
		...
		try {
			...
			if (rs.next()) {
				av = new Aviao(rs.getInt("qtd_turbinas"), rs.getFloat("capac_combust_por_turbina"),
						rs.getString("identificacao"), rs.getString("modelo"), rs.getInt("capac_passageiros"),
						rs.getDouble("capac_carga"));
