...
public class CidadeDAO {
	...
	public void inserirNoBanco(Cidade cid) {
		try {
			PreparedStatement pst = con
					.prepareStatement("INSERT INTO cidade(municipio,pais,estado) " + "VALUES (?,?,?)");
	...
	public void alterarNoBanco(Cidade cid) {
		try {
			PreparedStatement pst = con
					.prepareStatement("UPDATE cidade SET pais = ?," + " estado = ? ,municipio = ? WHERE idcidade = ?");
	...
	public Cidade consultar(long cod) {
		...
		try {
			PreparedStatement pst = con.prepareStatement(
					"SELECT idcidade, municipio, pais, estado " + "FROM cidade" + " WHERE idcidade = ? ");

			pst.setInt(1, (int) cod);
			rs = pst.executeQuery();

			if (rs.next()) {
				cid = new Cidade(rs.getInt("idcidade"), rs.getString("municipio"), rs.getString("pais"),
						rs.getString("estado"));
	...
	public ArrayList<Cidade> relatorio() {
		...
		try {
			...
			while (rs.next()) {
				...
				cidades.add(new Cidade(rs.getInt("idcidade"), rs.getString("municipio"), rs.getString("pais"),
						rs.getString("estado")));
			}
			return cidades;
